...
- (UITableView *)initWithTextField:(UITextField *)textField inViewController:(UIViewController *) parentViewController withOptions:(NSDictionary *)options
{
    ...
    if (textField.tag == 5000) {
        frame = CGRectMake(164/2, 162, 443/2, 200);
        //self.isStyle = @"yes";
    }else if (textField.tag == 10001)
    {
        frame = CGRectMake(164/2,418, 443/2, 200);//418
    }else if (textField.tag == 10002)
    {
        frame = CGRectMake(164/2,528, 443/2, 200);//528
    }else if (textField.tag == 10003)
    {
        frame = CGRectMake(164/2, 638, 443/2, 200);
    }else if (textField.tag == 10004)
    {
        frame = CGRectMake(164/2, 748, 443/2, 200);
    }
    ...
    UIView *mView = (UIView *)[parentViewController.view viewWithTag:9999999];
...
- (BOOL) substringIsInDictionary:(NSString *)subString
{
    ...
    NSRange range;
//    if ([self.isStyle isEqualToString:@"yes"]) {
//        for (int i = 0; i<self.suggestionsDictionary.count; i++) {
//            NSString *t_one = _suggestionsDictionary[i][@"name"];
//            range = ([[self.options valueForKey:ACOCaseSensitive] isEqualToNumber:[NSNumber numberWithInt:1]]) ? [t_one rangeOfString:subString] : [t_one rangeOfString:subString options:NSCaseInsensitiveSearch];
//            if (range.location != NSNotFound) [tmpArray addObject:t_one];
//
//        }
//    }else
//    {
...
//    if ([self.isStyle isEqualToString:@"yes"]) {
//        cell.textLabel.text = [self.suggestionOptions objectAtIndex:indexPath.row][@"name"];
//    }else
//    {
...
{
//    if ([self.isStyle isEqualToString:@"yes"]) {
//        [self.textField setText:[self.suggestionOptions objectAtIndex:indexPath.row][@"name"]];
//    }else
//    {
