...
@interface LinesMembersViewController ()

@end

@implementation LinesMembersViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        linesArray = [[NSMutableArray alloc] initWithCapacity:100];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    //机构表
    lineTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, [[UIScreen mainScreen] bounds].size.height-37-64) style:UITableViewStylePlain];
    lineTableView.delegate = self ;
    lineTableView.dataSource = self ;
    lineTableView.backgroundView = nil ;
    lineTableView.backgroundColor = [UIColor clearColor];
...
- (void)getDataWithNum:(NSInteger)num
{
    ...
    [BCHTTPRequest getGroupsDepartmentUsersWithDid:_departmentIdString WithPage:num usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
        if (isSuccess == YES) {
            if (num == 1) {
                linesArray = [[NSMutableArray alloc] initWithCapacity:100];
            }
            ...
            if ([resultDic[@"list"] count] > 0) {
                ...
                [linesArray addObjectsFromArray:resultDic[@"list"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.headerImageView setImageWithURL:[NSURL URLWithString:linesArray[indexPath.row][@"user_logo"]] placeholderImage:[UIImage imageNamed:@"ceshi@2x"]];
    ...
    cell.titleLabel.text = linesArray[indexPath.row][@"user_name"];
    cell.companyLabel.text = [NSString stringWithFormat:@"公司：%@",linesArray[indexPath.row][@"user_com"]];
...
- (void)clickHeadButton:(UIButton *)button
{
    ...
    mySweepVC.friendIdString = linesArray[button.tag][@"user_id"];
...
-(void)createHeaderView{
	...
	[lineTableView addSubview:_refreshHeaderView];
...
- (void)finishReloadingData{
	...
	if (_refreshHeaderView) {
        [_refreshHeaderView egoRefreshScrollViewDataSourceDidFinishedLoading:lineTableView];
    }
    
    if (_refreshFooterView) {
        [_refreshFooterView egoRefreshScrollViewDataSourceDidFinishedLoading:lineTableView];
        [self setFooterView];
    }
...
-(void)setFooterView{
    ...
    CGFloat height = MAX(lineTableView.contentSize.height, lineTableView.frame.size.height);
    ...
    if (_refreshFooterView && [_refreshFooterView superview])
	{
        ...
        _refreshFooterView.frame = CGRectMake(0.0f,
                                              height,
                                              lineTableView.frame.size.width,
    ...
    }else
	{
        ...
        _refreshFooterView = [[EGORefreshTableFooterView alloc] initWithFrame:
                              CGRectMake(0.0f, height,
                                         lineTableView.frame.size.width, self.view.bounds.size.height)];
        _refreshFooterView.delegate = self;
        [lineTableView addSubview:_refreshFooterView];
    }
