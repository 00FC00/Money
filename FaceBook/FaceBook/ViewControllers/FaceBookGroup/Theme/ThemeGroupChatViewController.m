//
//  ThemeGroupChatViewController.m
//  FaceBook
//
...
@interface ThemeGroupChatViewController ()
{
    //常用联系人
...
@implementation ThemeGroupChatViewController
...
- (void)viewDidLoad
{
    ...
    //    themeChatGroupReload
    //接受通知
    [[NSNotificationCenter defaultCenter] addObserverForName:@"themeChatGroupReload" object:nil queue:nil usingBlock:^(NSNotification *note) {
...
{
    [BCHTTPRequest getThemeTopContactsWithTheme_id:_themeIdString usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
        if (isSuccess == YES) {
...
{
    [BCHTTPRequest getThemeGroupUsersWithTheme_id:_themeIdString usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
        if (isSuccess == YES) {
...
- (void)clickaddGroupChat
{
    ...
    invitedThemePeopleVC.groupid = _themeIdString;
    invitedThemePeopleVC.fromString = @"主题群聊";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if (tableView.tag == 7007)
    {
        ...
        cMChatMainVC.messageWhereIds = self.themeIdString;
