...
@interface ThemeDynamicViewController ()

@end

@implementation ThemeDynamicViewController
...
- (void)viewDidLoad
{
    ...
    themeDynamicTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, [[UIScreen mainScreen] bounds].size.height-37-64) style:UITableViewStylePlain];
    themeDynamicTableView.delegate = self ;
    themeDynamicTableView.dataSource = self ;
    themeDynamicTableView.backgroundView = nil ;
    
    themeDynamicTableView.backgroundColor = [UIColor clearColor];
    themeDynamicTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    [self.view addSubview:themeDynamicTableView];
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:@"ThemeReload" object:nil queue:nil usingBlock:^(NSNotification *note) {
        pageNumber = 1;
        [self getDataWithNum:pageNumber];
    }];
...
{
    [BCHTTPRequest getGroupsThemeActionListWithTid:_themeIdString WithPage:num usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
        if (isSuccess == YES) {
            if (num == 1) {
                themeArray = [[NSMutableArray alloc] initWithCapacity:100];
            }
            ...
            [themeDynamicTableView reloadData];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([themeArray[indexPath.row][@"pictures"] count] > 0) {
        return 173;
    }else
    {
        return 93;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    pictureArray = themeArray[indexPath.row][@"pictures"];
    ...
    cell.markTitleLabel.text = themeArray[indexPath.row][@"user_name"];
    cell.markTimeLabel.text = themeArray[indexPath.row][@"add_time"];
    ...
    NSString *str1 = themeArray[indexPath.row][@"content"];
    CGSize size1;
    //***********ios7的方法
    if (IS_IOS_7)
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    faceBookDynamicDetialsVC.aidString = themeArray[indexPath.row][@"id"];
    faceBookDynamicDetialsVC.iIdString = _themeIdString;
    faceBookDynamicDetialsVC.fromString = @"主题动态";
...
-(void)createHeaderView{
	...
	[themeDynamicTableView addSubview:_refreshHeaderView];
...
- (void)finishReloadingData{
	...
	if (_refreshHeaderView) {
        [_refreshHeaderView egoRefreshScrollViewDataSourceDidFinishedLoading:themeDynamicTableView];
    }
    
    if (_refreshFooterView) {
        [_refreshFooterView egoRefreshScrollViewDataSourceDidFinishedLoading:themeDynamicTableView];
        [self setFooterView];
    }
...
-(void)setFooterView{
    ...
    CGFloat height = MAX(themeDynamicTableView.contentSize.height, themeDynamicTableView.frame.size.height);
    ...
    if (_refreshFooterView && [_refreshFooterView superview])
	{
        ...
        _refreshFooterView.frame = CGRectMake(0.0f,
                                              height,
                                              themeDynamicTableView.frame.size.width,
    ...
    }else
	{
        ...
        _refreshFooterView = [[EGORefreshTableFooterView alloc] initWithFrame:
                              CGRectMake(0.0f, height,
                                         themeDynamicTableView.frame.size.width, self.view.bounds.size.height)];
        _refreshFooterView.delegate = self;
        [themeDynamicTableView addSubview:_refreshFooterView];
    }
