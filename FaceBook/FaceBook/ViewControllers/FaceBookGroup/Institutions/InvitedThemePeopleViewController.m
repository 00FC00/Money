//
//  InvitedThemePeopleViewController.m
//  FaceBook
//
...
@interface InvitedThemePeopleViewController ()
{
    NSString *receiveId;
}
@end

@implementation InvitedThemePeopleViewController
        ...
        [BCHTTPRequest GetTheFaceBookInstitutionMembersWithInstitutionID:self.groupid WithPages:num usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
            if (isSuccess == YES) {
                ...
                for (int j = 0; j< [_listArray count]; j++) {
                    ...
                    FriendsItem* friendItem = [FriendsItem friendsItem];
...
- (void)ButtonClicked
{
    ...
    if ([_selectedFriendsArray count] > 0) {
        ...
        receiveId = @"";
        if (_selectedFriendsArray.count>0) {
            FriendsItem *receivefriends = _selectedFriendsArray[0];
        ...
        for (int i = 1; i< _selectedFriendsArray.count; i++) {
            FriendsItem *receivefriends = _selectedFriendsArray[i];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    FriendsItem* friendItem = [_friendsArray objectAtIndex:indexPath.row];
