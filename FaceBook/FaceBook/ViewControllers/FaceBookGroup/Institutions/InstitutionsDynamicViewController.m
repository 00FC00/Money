//
//  InstitutionsDynamicViewController.m
//  FaceBook
//
...
@interface InstitutionsDynamicViewController ()

@end

@implementation InstitutionsDynamicViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        //myToolArray = [[NSMutableArray alloc]initWithObjects:@"同行",@"照片",@"招聘",@"娱乐",@"电影",@"笑话",@"糗事",@"内涵",@"军事",@"国际", nil];
...
- (void)viewDidLoad
{
    ...
    //动态列表i_DynamicTableView;
    i_DynamicTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, [[UIScreen mainScreen] bounds].size.height-37-64) style:UITableViewStylePlain];
    i_DynamicTableView.delegate = self ;
    i_DynamicTableView.dataSource = self ;
    i_DynamicTableView.backgroundView = nil ;
    i_DynamicTableView.tag = 6007;
    i_DynamicTableView.backgroundColor = [UIColor clearColor];
    i_DynamicTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    [self.view addSubview:i_DynamicTableView];
    

    GlobalVariable *globalVariable = [GlobalVariable sharedGlobalVariable];
    if ([globalVariable.institutionis_memberString integerValue] == 1) {
        //获取数据
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:@"InstitutionReload" object:nil queue:nil usingBlock:^(NSNotification *note) {
        pageNumber = 1;
        [self getData:pageNumber];
    }];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        if ([listArray[indexPath.row][@"pictures"] count] > 0) {
            return 173;
        }else
        {
            return 93;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
        ...
        pictureArray = listArray[indexPath.row][@"pictures"];
        ...
        [cell.headerImageView setImageWithURL:[NSURL URLWithString:listArray[indexPath.row][@"user_logo"]] placeholderImage:[UIImage imageNamed:@"ceshi@2x"]];
        cell.markTitleLabel.text = listArray[indexPath.row][@"user_name"];
        cell.markTimeLabel.text = listArray[indexPath.row][@"add_time"];
    ...
    NSString *str1 = listArray[indexPath.row][@"content"];
    CGSize size1;
    //***********ios7的方法
    if (IS_IOS_7)
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView.tag == 6007)
    {
        ...
        faceBookDynamicDetialsVC.aidString = listArray[indexPath.row][@"id"];
        faceBookDynamicDetialsVC.fromString = @"机构动态";
...
{
     NSLog(@"########22%@",self.groupId);
    //获取数据
    [BCHTTPRequest getTheFaceBookInstitutionActionListWithPages:num WithInstitutionID:self.groupId usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
        ...
        if (isSuccess == YES) {
            if ([(NSArray *)resultDic[@"list"] count] > 0) {
                ...
                [listArray addObjectsFromArray:resultDic[@"list"]];
            }
            
            [i_DynamicTableView reloadData];
...
-(void)createHeaderView{
	...
	[i_DynamicTableView addSubview:_refreshHeaderView];
...
- (void)finishReloadingData{
	...
	if (_refreshHeaderView) {
        [_refreshHeaderView egoRefreshScrollViewDataSourceDidFinishedLoading:i_DynamicTableView];
    }
    
    if (_refreshFooterView) {
        [_refreshFooterView egoRefreshScrollViewDataSourceDidFinishedLoading:i_DynamicTableView];
        [self setFooterView];
    }
...
-(void)setFooterView{
    ...
    CGFloat height = MAX(i_DynamicTableView.contentSize.height, i_DynamicTableView.frame.size.height);
    ...
    if (_refreshFooterView && [_refreshFooterView superview])
	{
        ...
        _refreshFooterView.frame = CGRectMake(0.0f,
                                              height,
                                              i_DynamicTableView.frame.size.width,
    ...
    }else
	{
        ...
        _refreshFooterView = [[EGORefreshTableFooterView alloc] initWithFrame:
                              CGRectMake(0.0f, height,
                                         i_DynamicTableView.frame.size.width, self.view.bounds.size.height)];
        _refreshFooterView.delegate = self;
        [i_DynamicTableView addSubview:_refreshFooterView];
    }
