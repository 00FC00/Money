//
//  DynamicGroupChatViewController.m
//  FaceBook
//
...
@interface DynamicGroupChatViewController ()
{
    //常用联系人
...
@implementation DynamicGroupChatViewController
...
- (void)viewDidLoad
{
    ...
    //refreshGroupList
    //接受通知
    [[NSNotificationCenter defaultCenter] addObserverForName:@"refreshGroupList" object:nil queue:nil usingBlock:^(NSNotification *note) {
...
{
    [BCHTTPRequest GetTheFaceBookInstitutionGroupChatOftenConnectPeopleWithInstitutionID:self.groupID usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
        if (isSuccess == YES) {
...
{
    [BCHTTPRequest GetTheFaceBookGroupChatListWithInstitutionID:self.groupID usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
        if (isSuccess == YES) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if (tableView.tag == 7007)
    {
        ...
        cMChatMainVC.messageWhereIds = self.groupID;
