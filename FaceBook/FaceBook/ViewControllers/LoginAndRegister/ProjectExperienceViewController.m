...
@interface ProjectExperienceViewController ()
{
    ...
    UITextField *projectOfficeTextField;//项目职务
    UITextField *projectNameTextField;//项目名称
...
- (void)viewDidLoad
{
    ...
    if ([self.editStyle isEqualToString:@"edit"]) {
        companyTextField.text = self.projectDiction[@"com_name"];
    }
    ...
    projectOfficeTextField.placeholder = @"请填写项目职位";
    if ([self.editStyle isEqualToString:@"edit"]) {
        projectOfficeTextField.text = self.projectDiction[@"project_positions"];
    }
    ...
    projectNameTextField.placeholder = @"请填写项目名称";
    if ([self.editStyle isEqualToString:@"edit"]) {
        projectNameTextField.text = self.projectDiction[@"project_name"];
    }
    ...
    if ([self.editStyle isEqualToString:@"edit"]) {
        _contentTextView.text = self.projectDiction[@"project_intro"];
    }
    ...
    {
    _placeHolderLabel.text = @"请填写项目介绍...";
    }
    ...
    //起止时间
    UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, _contentTextView.frame.origin.y+_contentTextView.frame.size.height+20/2, 160/2, 72/2)];
    timeLabel.text = @"起止时间：";
    ...
    if ([self.editStyle isEqualToString:@"edit"]) {
        NSString *starStr = self.projectDiction[@"start_time"];
        [startTimeButton setTitle:starStr forState:UIControlStateNormal];
    }else
    {
    ...
    if ([self.editStyle isEqualToString:@"edit"]) {
        NSString *endStr = self.projectDiction[@"end_time"];
...
//-(void)timeButtonClicked:(UIButton *)sender
//{    
//    [backgroundScrollView setContentOffset:CGPointMake(0,600/2) animated:YES];
//    
...
{
    [BCHTTPRequest dellTheProjectItemWithProjectID:self.projectDiction[@"id"] usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
        if (isSuccess == YES) {
            //定义通知
...
//    if ([date1 timeIntervalSinceDate:now] && [date2 timeIntervalSinceDate:now]) {
...
//    }
    ...
    {
        [[DMCAlertCenter defaultCenter] postAlertWithMessage:@"请输入项目职务"];
    }else if ([projectNameTextField.text length]==0)
    {
        [[DMCAlertCenter defaultCenter] postAlertWithMessage:@"请输入项目名称"];
    }else if ([self.contentTextView.text length]==0)
    {
        [[DMCAlertCenter defaultCenter] postAlertWithMessage:@"请输入项目介绍"];
...
//        //修改资料
        [BCHTTPRequest modifyTheProjectItemWithProjectID:self.projectDiction[@"id"] WithCompanyName:companyTextField.text WithProjectPost:projectOfficeTextField.text WithProjectName:projectNameTextField.text WithProjectIntro:self.contentTextView.text WithStartTime:startTimeButton.titleLabel.text WithEndTime:endTimeButton.titleLabel.text usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
            if (isSuccess == YES) {
    ...
    {
        //新建的资料
        [BCHTTPRequest addTheProjectItemWithCompanyName:companyTextField.text WithProjectPost:projectOfficeTextField.text WithProjectName:projectNameTextField.text WithProjectIntro:self.contentTextView.text WithStartTime:startTimeButton.titleLabel.text WithEndTime:endTimeButton.titleLabel.text usingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
            if (isSuccess == YES) {
