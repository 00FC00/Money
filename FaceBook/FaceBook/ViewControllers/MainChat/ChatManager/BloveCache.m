...
#if DEBUG
#define CHECK_FOR_BloveCACHE_PLIST() if([key isEqualToString:@"BloveCache.plist"]) { \
NSLog(@"BloveCache.plist is a reserved key and can not be modified."); \
return; }
#else
#define CHECK_FOR_BloveCACHE_PLIST() if([key isEqualToString:@"BloveCache.plist"]) return;
#endif
...
- (id)init {
	...
	NSString* oldCachesDirectory = [[[cachesDirectory stringByAppendingPathComponent:[[NSProcessInfo processInfo] processName]] stringByAppendingPathComponent:@"BloveCache"] copy];
	...
	cachesDirectory = [[[cachesDirectory stringByAppendingPathComponent:[[NSBundle mainBundle] bundleIdentifier]] stringByAppendingPathComponent:@"BloveCache"] copy];
...
- (id)initWithCacheDirectory:(NSString*)cacheDirectory {
	if((self = [super init])) {
		...
		_cacheInfo = [[NSDictionary dictionaryWithContentsOfFile:cachePathForKey(_directory, @"BloveCache.plist")] mutableCopy];
...
- (void)setNeedsSave {
	dispatch_async(_cacheInfoQueue, ^{
		...
		dispatch_after(popTime, _cacheInfoQueue, ^(void){
			if(!_needsSave) return;
			[_cacheInfo writeToFile:cachePathForKey(_directory, @"BloveCache.plist") atomically:YES];
			_needsSave = NO;
		});
	});
}
