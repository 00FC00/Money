...
@interface TownViewController ()<UITableViewDataSource,UITableViewDelegate>
@property(nonatomic,strong)UITableView* myTableView;
@property (strong, nonatomic) NSMutableArray *allTownsArray;
@end
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.allTownsArray = [[NSMutableArray alloc]initWithCapacity:100];
    }
    return self;
}
- (void)viewDidLoad
{
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"STYPE"] integerValue] == 1) {
        NSLog(@"已经完善");
        //获取数据
        [BCHTTPRequest GetTheSameCityListWithUsingSuccessBlock:^(BOOL isSuccess, NSDictionary *resultDic) {
            if (isSuccess == YES) {
                self.allTownsArray = resultDic[@"list"];
    ...
    //self.title=@"新的好友";
    [[NSNotificationCenter defaultCenter]addObserverForName:@"showtown" object:nil queue:nil usingBlock:^(NSNotification *note) {
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return self.allTownsArray.count;
}
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    dic = self.allTownsArray[indexPath.row];
...
- (void)clickPassButton:(UIButton *)button
{
    ...
    diction = self.allTownsArray[button.tag - 4000];
